define(['bookmarks-compare', 'bookmarks-json_to_string'], function(compare, jsonToString){
  ...
  BookmarksParser.prototype.init = function(opt_options){
    this._setOptions(opt_options || {});
    this._attachInput();
  };
  
  BookmarksParser.prototype._setOptions = function(options){
    this._options = {
      id: options.id || '',
  ...
  BookmarksParser.prototype._attachInput = function(){
  ...
  BookmarksParser.prototype._findInput = function(){
    var input = null;
  ...
  BookmarksParser.prototype._createInput = function(){
  ...
  BookmarksParser.prototype._onInputChange = function(){
    var reader;
  ...
  BookmarksParser.prototype._readFile = function(event){
  ...
  BookmarksParser.prototype._parseLine = function(line){
    line = line.trim();
  ...
  BookmarksParser.prototype._addToHead = function(line){
    if (this._bookmarks.length === 1) this._newFile.head.push(line);
  };
  
  BookmarksParser.prototype._addTitle = function(line){
    var matches = {
      title: line.match(/>([^<^>]+)<\//),
  ...
  BookmarksParser.prototype._addFolder = function(line){
    this._folderLevel = this._fileBody;
  ...
  BookmarksParser.prototype._addLink = function(line){
    var matches = {
      title: line.match(/>([^<^>]+)<\//),
      url: line.match(/HREF="([^"]*)"/),
      added: line.match(/ADD_DATE="([^"]*)"/),
  ...
  BookmarksParser.prototype._closeFolder = function(){
    this._breadcrumbs.pop();
  };
  
  BookmarksParser.prototype._compareAfterFinish = function(){
  ...
  BookmarksParser.prototype._arrayToString = function(array){
    return array.join('\n');
  };
  
  BookmarksParser.prototype._downloadFile = function(){
    this._updateInput();
    this._showDownloadLink();
  };
  
  BookmarksParser.prototype._updateInput = function(){
    this._createDownloadLink();
    this._textToUrl();
  };
  
  BookmarksParser.prototype._textToUrl = function(){
  ...
  BookmarksParser.prototype._createDownloadLink = function(){
  ...
  BookmarksParser.prototype._showDownloadLink = function(){
    if (!this._downloadLink) return;
    
    this._downloadLink.classList.remove('hidden');
  };
  
  BookmarksParser.prototype._hideDownloadLink = function(){
    if (!this._downloadLink) return;
